# ミニゲーム集(タンクバトル × スロットカー × 麻雀)

ブラウザで動作する、最大4人参加のパーティーミニゲーム集。
Node.js製サーバーをローカルで起動し、ngrokで外部公開してURLを共有する想定です。

## デザイン / ユーザビリティ

- **共通**: デザイントークンで統一したUI(ボタン/カード/フォーカス枠)、レスポンシブ、接続状態インジケータ、ロビーの**招待URLコピー＋QRコード**。
- **麻雀**: 手番/鳴きのカウントダウンタイマー、最終捨牌ハイライト、鳴きボタンの牌プレビュー、中央の局・本場・リーチ棒表示、和了結果に**和了形の牌表示＋点棒移動アニメ**。本格的なSVG牌(筒子の丸/索子の竹)と卓向きレイアウト。**ツモ切り/手出しの区別**、**鳴き牌の向き(誰から鳴いたか)**、**テンパイ/フリテン表示**、**現在順位**、スマホ縦レイアウト対応。ルールに**流し満貫**を追加。
- **演出**: WebAudioの効果音(打牌/ツモ/ロン/リーチ/開始)、自分の手番・和了時のバイブ、開始バナー、サウンドのミュート切替。

## 現在の進捗

- [x] server/game1.js — ミニゲーム1(タンク×ボンバーマン)のサーバーロジック
- [x] server/game2.js — ミニゲーム2(スロットカー)のサーバーロジック
- [x] server/mahjong-core.js — 麻雀の点数計算エンジン(和了判定・役・符・点数表)
- [x] server/mahjong.js — ミニゲーム3(麻雀・東風戦)の卓進行エンジン
- [x] server/index.js — ロビー管理・WebSocket・進行管理
- [x] public/index.html, public/css/style.css — 画面の土台(ロビー/ゲーム選択/観戦/結果)
- [x] public/js/main.js — クライアント側の描画・入力処理(**PixiJS(WebGL)＋麻雀UIで実装済み**)
- [x] public/js/vendor/pixi.min.mjs — 同梱した PixiJS 本体(CDN非依存・自前サーバーで配信)

## セットアップ

```bash
npm install
npm start
```

http://localhost:3000 が起動します。

### 外部共有(ngrok)

ngrok は winget で導入できます(`winget install Ngrok.Ngrok`)。認証トークンは
`ngrok config add-authtoken <token>` で一度登録すれば以降不要です。

`~/AppData/Local/ngrok/ngrok.yml`(Windows)に名前付きトンネルを定義してあるので、
サーバーを起動した状態で:

```bash
ngrok start minigame
```

アカウントの静的ドメインに固定しているため、再起動しても URL は変わりません。
発行された `https://...ngrok-free.dev` を参加者に共有してください。
クライアントは `location` から `wss://` を自動判定するため追加設定は不要です。

注意点:

- 無料プランでは初回アクセス時に ngrok の警告ページが出ます。参加者は
  **「Visit Site」を一度押す**だけで通過できます。
- サーバーに認証はありません。URL を知っていれば誰でも参加/観戦できるので、
  遊ばないときは ngrok を停止してください。

## クライアント / 描画について

`public/js/main.js`(実装済み)がカバーする範囲:

1. WebSocket接続(`ws://` or `wss://` を `location` から自動判定)
2. ロビー・**ゲーム選択画面**・**観戦モード**・結果画面のUI制御(`join` / `spectate` / `to_select` / `to_lobby` / `pick` / `back_to_select`)
3. ミニゲーム1: `#g1-canvas` に `game1_state` を描画。矢印キー/SHIFT/SPACE、**または画面上の方向パッド＋ボム/発射ボタン**(マウス/タッチ)で操作
4. ミニゲーム2: `#g2-canvas` に `game2_state` を描画(＋タコメーター `#g2-tacho`)。SHIFT、**または画面上のアクセルボタン/コース長押し**(マウス/タッチ)で加速

※ 3ゲームとも**マウス/タッチだけで操作可能**です(キーボード任意)。
5. ミニゲーム3(麻雀): `#mj-table` に `game3_state` を DOM で描画。手牌タップで打牌、鳴き・立直・和了はボタン。`{type:"mahjong", action:{...}}` で送信

サーバー側メッセージ仕様は `server/index.js` を参照してください。

## ミニゲーム3:麻雀(東風戦)

本格的な4人打ちリーチ麻雀です。

- **対局**: 東風戦(東1〜4局)。親の連荘・本場・供託(リーチ棒)・飛び(誰かが0点未満で終局)に対応。
- **ルール**: 標準的なネット麻雀。**赤ドラあり**(赤5×3)、リーチ・一発・裏ドラ・ドラ・喰いタンあり。
- **点数計算**: `server/mahjong-core.js` が符・翻・点数を正しく計算(平和ツモ20符、七対子25符、満貫〜役満、親/子・ロン/ツモの支払い分け、本場・供託の加算)。役満(国士無双・大三元・四暗刻・字一色・九蓮宝燈ほか)も判定。
- **操作**: ツモ/ロン/ポン/チー/カン(明槓・暗槓・加槓)/リーチ/パス。フリテン(自河・リーチ後・同巡)判定あり。
- **空席はCPU(強さ設定あり)**: 選択画面でCPUの強さを **弱(ツモ切り)/中(牌効率＋リーチ)/強(＋リーチ者へのベタオリ)** から選べます。人が抜けた席もCPUが引き継いで対局を継続。
- **ダブロン/トリロン**: 同一牌への複数ロンに対応(本場・供託は頭ハネの和了者へ)。三家和(3人ロン)は流局(親流れなし)。
- **現在の役・待ち牌**: テンパイ時、待ち牌と「その牌で和了した場合の役」を表示。リーチ後の待ちも確認できます。
- **観戦**: 観戦者は**全席の手牌を表向きで確認**できます(各プレイヤーには自分の手牌のみ表示)。
- **自動整列(理牌)**: 手牌は常にソート表示され、変化時は牌が新しい位置へ滑らかに移動(FLIPアニメ)。
- **役一覧・点数表**: 対局画面の「📖 役・点数」ボタンから、実装役の一覧(翻数・食い下がり付き)と符×翻の点数早見表(子/親・ロン/ツモ)を表示。点数表は点数エンジンと同じ計算式から生成しているため実際の計算と一致します。

点数エンジンは `mahjong-core.js` として独立し、既知手の点数(21ケース)で検証済みです。

### 描画技術(PixiJS + Canvas 2D ハイブリッド)

ゲーム画面は **PixiJS(WebGL)** でGPU描画します。

- **スプライト**: 精密なベクター戦車/レーシングカー(Canvas 2Dで描いた図形)を、プレイヤー色ごとに **一度だけGPUテクスチャへ焼き込み**、以降は PixiJS のスプライトとして合成します。
- **状態補間**: サーバーは約30Hzで状態を配信。クライアント側で位置・角度を指数スムージングし、ディスプレイのリフレッシュレート(60fps等)で滑らかに描画。
- **エフェクト**: ボム爆発の加算合成グロー、スピンアウト時の火花、戦車/車の影・カラーグローなど。
- **マップ/トラック/ボム/弾** は `PIXI.Graphics`、**タコメーター** は専用の Canvas 2D ゲージで描画。

PixiJS は `public/js/vendor/pixi.min.mjs` に同梱し、自前サーバーから配信します(CDN非依存・オフライン動作可)。`main.js` は ES モジュールとして読み込まれ(`<script type="module">`)、サーバーは `.mjs` を `text/javascript` で配信します。
